...
class GrayscaleKinematicsObservation(ObservationType):
    FEATURES = ['presence', 'x', 'y', 'vx', 'vy']
    def __init__(self, env,
                 features=FEATURES,
                 vehicles_count=5,
    ...
    def space(self):
        try:
            image = spaces.Box(shape=self.shape, low=0, high=1, dtype=np.float32)
            kine = spaces.Box(shape=(self.vehicles_count, len(self.features)), low=-1, high=1, dtype=np.float32)
            return spaces.Dict({'image': image, 'kinematics': kine})
        except AttributeError:
            return None

    def _record_to_grayscale(self):
        raw_rgb = self.env.render('rgb_array')
        return np.dot(raw_rgb[..., :3], self.weights)
    ...
    def observe(self):
        ...
        self.state = np.roll(self.state, -1, axis=-1)
        self.state[:, :, -1] = new_obs
        return {'kinematics': obs, 'image': self.state}
...
def observation_factory(env, config):
    ...
    elif config["type"] == "GrayscaleAndKinematics":
        return GrayscaleKinematicsObservation(env, **config)
    else:
