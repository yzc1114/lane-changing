...
class Agent(object):
    ...
    def parse_args(args):
        ...
        parser.add_argument('--learner_type', type=str, default='DQN',
                            help="Algorithm to train from {PPO, A2C, DQN, DDPG, TD3}")
        parser.add_argument('--parallels', type=int, default=1)
        parser.add_argument('--nb_steps', type=int, default=2000*50, help="Number of training steps")
        parser.add_argument('--eval_interval_steps', type=int, default=1000, help="Eval and checkpoint interval steps")
    ...
    def train(self, args=None):
        ...
        checkpoint_callback = CheckpointCallback(save_freq=args.eval_interval_steps, save_path=weights_directory,
                                                 name_prefix=learner_name)
        # Separate evaluation env
        eval_env = make_env(1, obs_type)
        eval_callback = EvalCallback(eval_env, best_model_save_path=os.path.join(weights_directory, learner_name + '_best'),
        ...
        print(f"nb_steps: {nb_steps}, learner name: {learner_name}, obs_type: {obs_type}")
